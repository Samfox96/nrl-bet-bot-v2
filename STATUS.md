...
## Last Updated
**2026-07-03** — Stage 1 (xTry model) started. Wired two of Component 8's previously-dead situational inputs from real data: `due_flag_severity` and `scored_last_game`. These were "dead but present" — Component 8's logic existed and was unit-tested, but `generate_predictions.py`'s `build_raw_scores()` call stopped at `team_games_played`, so every context factor silently defaulted to neutral on every run. `due_flag_severity` now flows from the SAME `due_watch_by_team` entries that feed the email's DUE badge (so the try-probability boost and the displayed badge can no longer contradict each other — the whole point of the fix); `scored_last_game` is derived from each player's own last played game. Mapping is evidence-based: observed real composite range is [-0.539, +0.539], so severity = clip(composite, 0, 1) applied only to positive composites — a genuinely-due proven scorer gets a proportional boost (max ~1.16x), a non-due player stays neutral. **Verified in the live pipeline**: a real `workflow_dispatch` run for Round 18 (team-list source `committed_csv`, 5 fixtures ok) produced boosts on due players (e.g. Dolphins: Kulikefu Finefeuiaki due_flag sev=0.22 → x1.066) while non-due players stayed at 1.0 — confirmed by reproducing `build_raw_scores` against the committed R18 data, since `predictions_current.json` does not persist per-player component notes. **Deliberately still neutral (not fabricated)**: `games_since_return_from_injury` (only approximable by a round gap that conflates injury/drop/rest — an already-documented open limitation), `games_since_rep_return` (needs an Origin/Test schedule that doesn't exist in the repo), and `was_dropped_and_recalled` (needs a selection-vs-injury signal stats alone don't carry). Also this session: recovered a `generate-predictions.yml` that was accidentally overwritten with Python source (the SAME class of wrong-file-paste incident as the 2026-06-25 `weekly-update.yml` one — restored from the last-good commit; `scripts/generate_predictions.py` itself was unaffected).
...
| `match_data_FINAL_fixed.csv` | 1,169 | 2021–2026, 2026 through Round 17 | ✅ CURRENT (was stuck at Round 10 until 2026-06-25's backfill) |
